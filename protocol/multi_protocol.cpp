...
void AT_IT(unsigned char*pib , unsigned char* crsseed2 , unsigned char* resp ,unsigned char* pk,\
unsigned char *sk, unsigned char** query , unsigned char** crsseed , unsigned char** pi1){
    clock_t time1 = clock();
    ...
    std::cout << "Private bits in TokenIssue: " << std::endl;
...
//remove error term
        ...
        std::cout << "verify Pi1: " << std::endl;
        for(int i = 0 ; i < N_BATCH; i++){
            std::cout << Verify1(crsseed[i], pi1[i] , A , c->vecarray[i]) << std::endl;
    ...
    int loop = 0;
    while(!loop){
         loop = Prove_batch(pib , crsseed2 , bit , s0, s1 , e0 , e1 , e3 , h , A , t0, t1 , c);
...
void AT_CF(unsigned char** msg , unsigned char* pk , unsigned char*resp ,unsigned char**H1input, unsigned char** xchar ,\
    unsigned char** query , unsigned char* pib , unsigned char* crsseed2){
...
//recover c
    ...
    std::cout << "verify batch Pi: " << Verify_batch(crsseed2 , pib , A , c , t0 , t1 , h) << std::endl;
    // clock_t t3 = clock();
...
int main(){
    ...
    for(int i = 0; i < N_BATCH ; i++){
     ...
     AT_IT(pib , crsseed2 , resp , pk , sk , query , crsseed , pi1);
     AT_CF(msg , pk , resp , H1input , xchar , query , pib , crsseed2);
    std::cout << " Readbit: " << std::endl;
    for(int i=0; i<N_BATCH; i++){
        // std::cout <<"msg in Rb " << std::endl;    
        // printfBstr(msg[i],10);
